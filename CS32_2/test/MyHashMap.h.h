...
public:
    ...
    void associate(std::string key, const ValueType& value)
    {
		...
		{
			keyValue += (i+3) * key[i]; //use the ASCII value to gen key
		}
		...
		{
			if (curr->key == key) //key already exists and just needs update
			{
				curr->val = value;
				return;
			}
    ...
    const ValueType* find(std::string key) const
    {
		...
		{
			keyValue += i * key[i]; //use the ASCII value to gen key
		}
		...
		{
			if (checkThis->key == key) //found the key
