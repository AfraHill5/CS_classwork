...
{
	if (s.size() == 2  &&  comp(s[1], s[0])) //case: 2 values
        swap(s[0], s[1]);
	...
	for (int currSize = 2; currSize <= s.size(); currSize++)
	{
		Student toBeSorted = s[currSize-1]; //the current student we are looking at
		int i = currSize-2;
		while (i >= 0 && comp(toBeSorted, s[i])) 
		{
			s[i+1] = s[i];
			i--;
		} //shifts all the elements over until finding the correct spot for toBeSorted
		s[i+1] = toBeSorted; //inserts the student to the correct spot
	}
}
...
int main()
{
	...
	for (int i = 0; i < auxStuds.size(); i++)
		studPtrs.push_back(&auxStuds[i]);
