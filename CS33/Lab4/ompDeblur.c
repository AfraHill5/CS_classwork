...
void OMP_GaussianBlur(double *u, double Ksigma, int stepCount)
{
	...
	{
		#pragma omp parallel for shared(xMax, yMax, zMax) private(x,y,z)
		for(z = 0; z < zMax; z++) 
		{
			for(y = 0; y < yMax; y++)
			{
		...
		#pragma omp parallel for shared(yMax, xMax) private(x,y)
		for(y = 0; y < yMax; y++)
		{
			for(x = 0; x < xMax; x++)
			{
		...
		#pragma omp parallel for shared(xMax, yMax,zMax) private (x,y)
		for(y = 0; y < yMax; y++)
		{
			for(x = 0; x < xMax; x++)
			{
	...
	#pragma omp parallel for private(z,y,x) shared (zMax, yMax, xMax, postScale, u)
	for(z = 0; z < zMax; z++)
	{
		for(y = 0; y < yMax; y++)
		{
