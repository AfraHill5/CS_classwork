...
void Game::play()
{
    ...
    do
    {
        ...
        msg = "";
        cout << endl;
        cout << "Move (u/d/l/r/su/sd/sl/sr/c//h/q): "; //added h for history command
        string action;
        getline(cin,action);
        if (action.size() == 0)
            p->stand();
        ...
        {
			History h = m_arena->history(); //gets the history from the arena
            switch (action[0])
            {
			  ...
			  case 'h': 
				  ...
				  cin.ignore(10000, '\n');
				  continue;
				  //this calls the history function, tells the user to hit enter to continue and waits for input, returns when user hits enter
