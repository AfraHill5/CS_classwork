...
bool compareStorePtr(const Store* lhs, const Store* rhs)
{
    ...
    //       how you can do it in one line by calling compareStore. DONE
...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
	...
	int i = 0;
	while (i < s.size() - 1) //stops when it gets to the final store
	{
		if (comp(s[i+1], s[i])) //if the next item should be in front of the first
		{
			Store temp = s[i+1];
			for (i; i >= 0 && comp(temp, s[i]); i--)
				s[i+1] = s[i]; //shifts all the values down one
			s[i+1] = temp;
			i = 0; //start over and go again
		}
		else 
			i++; //keep going
	}
...
int main()
{
      ...
      // destruction time.
    {
	 ...
	 for (int i = 0; i < auxStores.size(); i++)
		 storePtrs.push_back(&auxStores[i]); //assigns a pointer to every value of auxStores


      // TODO:  Sort the vector of pointers using the STL sort algorithm
      //        with compareStorePtr as the ordering relationship. DONE
