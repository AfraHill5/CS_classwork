//Zach North
//603 885 768
//Homework 2, Problem 3 -- Mazequeue function
...
void determineDistances(const char maze[][10], int sr, int sc, int dist[][10])
{
	...
	queue<Coord> distQueue; //initializes a new stack to get the distances

	Coord originCoord(sr, sc); //initializes the first coordinate

	distQueue.push(originCoord); //pushes the starting coordinates onto the stack
	while (!distQueue.empty())
	{
		Coord currentCoord = distQueue.front(); //sets the current coordinate to the top coordinate on the stack
		distQueue.pop();
		...
		if (maze[r-1][c] == '.') //if you can move up
		{
			if (dist[r-1][c] > v+1) //if the distance value is greater than the current value +1
			{
				...
				Coord temp(r-1,c);
				distQueue.push(temp); //pushes the next coord onto the stack
			}
		}
