...
/* Added by me. Make sure to build with -lpthreads option. */
 #include <pthread.h> 
...
/* vars needed for proper parallization of the loop: */
...
void* threadTask(void* args)
{
    ...
    /* have to "decode" the passed in arguments */
    struct InfoStruct *input_data;
    input_data = (struct InfoStruct *) args;
    
    int ws = input_data->iterWidthStart;
    int wl = input_data->iterWidthStart + input_data->iterWidthLen;
    int hs = input_data->iterHeightStart;
    int hl = input_data->iterHeightStart + input_data->iterHeightLen;
    ...
    const double pixel_dx = input_data->pixel_dx;
    const double pixel_dy = input_data->pixel_dy;
    const double subsample_dx = input_data->subsample_dx;
    const double subsample_dy = input_data->subsample_dy;
    scene_t scene = input_data->scene;


    for( int px=ws; px < wl; ++px )
    {
        const double x = pixel_dx * ((double)( px-(width/2) ));
        for( int py=hs; py < hl; ++py )
        {
...
main( int argc, char **argv )
{
    ...
    struct InfoStruct* is;
    pthread_t threads[nthreads];
    int wlen = width / nthreads;
    int hlen = height / nthreads;
    /* split the work equally among threads */
    
    is = malloc(sizeof(struct InfoStruct[nthreads]));
    int rc;
    for (int i = 0; i < nthreads; i++)
    {
        /* Load this up with the information each thread will need to 
         * run the task loop independently of the others. */
        is[i].iterWidthLen = wlen;
        is[i].iterWidthStart = i * wlen; /*where to start in the loop*/
        is[i].iterHeightLen = hlen;
        is[i].iterHeightStart = i * hlen;
        is[i].pixel_dx = pixel_dx;
        is[i].pixel_dy = pixel_dy;
        is[i].subsample_dx = subsample_dx;
        is[i].subsample_dy = subsample_dy;
        is[i].scene = scene;
        /* create each thread, passing in the info struct */
        rc = pthread_create(&threads[i], NULL, threadTask, (void*) &is[i]);
    }
