...
//---------------------------------------------------------------------------------------------------
//Thing
const int numThingVertices = 12;

vec4 tPoints [numThingVertices];
vec3 tNormals[numThingVertices];
vec2 tUV     [numThingVertices];

// Vertices of a sc, centered at origin
vec4 tVertices[4] = {
...
// thing generates 1 triangle for each face
int i = 0;
void thing( int a, int b, int c, const vec3& normal )
{
    tPoints[i] = tVertices[a]; tNormals[i] = normal; 
    tUV[i] = vec2(0.0f, 1.0f); i++;
    tPoints[i] = tVertices[b]; tNormals[i] = normal;
    tUV[i] = vec2(0.0f, 0.0f); i++;
    tPoints[i] = tVertices[c]; tNormals[i] = normal;
    tUV[i] = vec2(1.0f, 0.0f); i++;
}

// generate 12 triangles: 36 vertices, 36 normals, 36 texture coordinates
void colorThing()
{
    thing( 0, 1, 2, vec3( 0.0f,  1.0f,  0.0f) );
    thing( 2, 3, 0, vec3( 0.0f,  0.0f,  1.0f) );
    thing( 1, 2, 3, vec3( 1.0f, 0.0f,  1.0f) );
    thing( 0, 1, 3, vec3( -1.0f,  -1.0f,  1.0f) );
}

// initialization
void generateThing(GLuint program, ShapeData* thingData)
{
    colorThing();
	thingData->numVertices = numThingVertices;

    // Create a vertex array object
    glGenVertexArrays( 1, &thingData->vao );
    glBindVertexArray( thingData->vao );

    // Set vertex attributes
    setVertexAttrib(program, 
        (float*)tPoints,  sizeof(tPoints), 
        (float*)tNormals, sizeof(tNormals),
        (float*)tUV,      sizeof(tUV));
}
